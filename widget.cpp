    ...
    , ui(new Ui::Widget)
{
    ...
    this->setWindowTitle("China Stock K chart");
    initChart(QString,QDate);
}
...
void Widget::initChart(QString stock,QDate date)
{
    QCandlestickSeries* series = new QCandlestickSeries();   // 创建一个蜡烛图数据集对象，用于绘制蜡烛图
    int Year = date.year();
    int Month = date.month();
    char y[6]={0},m[4]={0};
    itoa(Year,y,10),itoa(Month,m,10);
    name = yy;
    name += "年";
    name += mm;
    name += "月";
    name += " 股票走势";
    series->setName("股票走势");                           // 设置名称
    ...
    ui->chartView->chart()->setTitle("China stock K chart");            //设置图表标题
